  ...
  renderHTML: function(container, venta) {
      ...
      <div class="col-lg-6">
        <div class="card card-elevated no-print position-relative overflow-hidden">
          ...
          <div style="background:${isAnulada ? 'var(--text-muted)' : 'var(--primary)'};padding:1.5rem;text-align:center;color:white">
            ...
            <h3 style="font-weight:700;margin:0">${(mercado.nombre || 'MINIMARKET').toUpperCase()}</h3>
            <p style="opacity:.75;font-size:.85rem;margin:0">RUC: ${mercado.ruc || '20123456789'}</p>
            <p style="opacity:.75;font-size:.85rem;margin:0">${mercado.direccion || ''}</p>
          </div>
          <div class="card-body p-4 p-md-5">
            <div style="text-align:center;margin-bottom:1rem;border-bottom:1px solid var(--border);padding-bottom:1rem">
              <h5 style="font-weight:700;color:var(--primary);margin-bottom:4px">${venta.tipo_comprobante}</h5>
              <h6 style="font-weight:700">${venta.serie || ''}-${String(venta.numero || '').padStart(6, '0')}</h6>
            ...
            <div style="font-size:.85rem;margin-bottom:1rem;padding:0.75rem 1rem;background:var(--surface-hover);border-radius:var(--radius)">
              ...
              <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="color:var(--text-muted)">NOMBRE:</span><span style="font-weight:600">${venta.cliente.nombre}</span></div>
              <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="color:var(--text-muted)">${venta.cliente.tipo_documento || 'DOC'}:</span><span style="font-weight:600">${venta.cliente.num_documento}</span></div>
              ${venta.cliente.direccion ? `<div style="display:flex;justify-content:space-between"><span style="color:var(--text-muted)">DIRECCIÓN:</span><span style="font-weight:600;text-align:right;max-width:200px">${venta.cliente.direccion}</span></div>` : ''}
            </div>` : ''}

            <div style="font-size:.85rem;margin-bottom:1rem">
              <div style="display:flex;justify-content:space-between;margin-bottom:4px"><span style="font-weight:600;font-size:.7rem;text-transform:uppercase;color:var(--text-muted)">Vendedor:</span><span style="font-weight:600">${venta.usuario ? (venta.usuario.first_name || venta.usuario.username) : ''}</span></div>
              <div style="display:flex;justify-content:space-between"><span style="font-weight:600;font-size:.7rem;text-transform:uppercase;color:var(--text-muted)">Método de Pago:</span><span style="font-weight:600">${venta.metodo_pago}</span></div>
            </div>
            ...
            <div style="background:var(--surface-hover);border-radius:var(--radius);padding:1rem;margin-bottom:1rem;border:1px solid var(--border)">
              ...
              <div style="border-top:1px solid var(--border);padding-top:12px;display:flex;justify-content:space-between">
                ...
                <span style="font-weight:600;font-size:.85rem">${venta.num_operacion || '---'}</span>
              </div>`}
            </div>
    ...
    <div class="print-ticket">
      <div class="text-center">
        <div class="fw-bold" style="font-size:11pt">${(mercado.nombre || 'MINIMARKET').toUpperCase()}</div>
        <div>RUC: ${mercado.ruc || '20123456789'}</div>
        <div style="font-size:7pt">${(mercado.direccion || '').toUpperCase()}</div>
        <div class="ticket-divider"></div>
        <div class="fw-bold">${venta.tipo_comprobante}</div>
        <div class="fw-bold">${venta.serie || ''}-${String(venta.numero || '').padStart(6, '0')}</div>
        <div class="ticket-divider"></div>
      </div>
      <div style="font-size:7.5pt">
        <div>FECHA: ${Utils.formatDateTime(venta.fecha_hora)}</div>
        <div>CAJERO: ${venta.usuario ? (venta.usuario.username || '').toUpperCase() : ''}</div>
        ${venta.cliente ? `<div class="ticket-divider"></div><div>CLIENTE: ${venta.cliente.nombre.toUpperCase()}</div><div>${venta.cliente.tipo_documento || 'DOC'}: ${venta.cliente.num_documento}</div>` : ''}
        <div class="ticket-divider"></div>
      </div>
      ...
      <table style="width:100%;font-size:7.5pt">
        ...
        ${venta.metodo_pago === 'Efectivo'
          ...
          : `<tr><td class="text-right">NRO. OP:</td><td class="text-right">${venta.num_operacion || '---'}</td></tr>`}
      </table>
    ...
    if (venta.detalles) {
      ...
      venta.detalles.forEach(d => {
        html += `<tr><td style="font-weight:500">${d.producto ? d.producto.nombre : d.producto_nombre || ''}<br><small style="color:var(--text-muted)">${Utils.formatMoney(d.precio_unitario)} c/u</small></td><td class="text-center">${d.cantidad}</td><td class="text-center">${Utils.formatMoney(d.precio_unitario)}</td><td class="text-end fw-bold">${Utils.formatMoney(d.subtotal)}</td></tr>`;
        printHtml += `<tr><td style="padding:2px 0">${d.cantidad}</td><td style="padding:2px 0">${d.producto ? d.producto.nombre.toUpperCase() : (d.producto_nombre || '').toUpperCase()}</td><td class="text-right" style="padding:2px 0">${Number(d.subtotal || 0).toFixed(2)}</td></tr>`;
      });
